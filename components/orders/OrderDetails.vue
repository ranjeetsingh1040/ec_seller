  ...
  <g>
    <path style="fill:#fff;" d="M15.529,22.539c-0.49,0-0.888-0.397-0.888-0.887V9.406c0-0.49,0.398-0.888,0.888-0.888
      ...
      <div v-if="page_count > 1">
        <paginate :page-count="totalPages" :page-range="3" :margin-pages="1"
      ...
      <div class="flex flex-col lg:flex-row md:flex-row my-4">
        ...
        <div class="flex flex-1">
          ...
          <!-- <p class="m-1" >Page</p>
       <a  class="border bg-gray-100 px-2" @click="minus()">-</a>
       <input type="text" name="name" v-model="orderpage" class="w-6 border mx-1 text-xs">
       <a   class="border bg-gray-100 px-1" @click="add()">+</a>
       <p class="m-1 border-r pr-3 border-black">of {{orderpage}} pages </p> -->
        ...
        <option value="20">20</option>
          ...
          <div class="flex m-1" style="display: none;">
            <svg class="w-4 h-4 fill-current" style="color:#ff5139;" height="1063pt" viewBox="-33 -100 1063.7506 1063"
              width="1063pt" xmlns="http://www.w3.org/2000/svg">
              <path
        ...
        <div class="flex">
          ...
          <button class="is-info rounded text-xs p-1 mx-1" @click="getOrders()">Search</button>
            ...
            <tr v-for="order in orders" :key="order.id">
              ...
              <td>
                ...
                <div v-if="order.status !== 'cancel'">
                  <span class="mx-2">
                    <nuxt-link :to="{ name: 'orders-orderid', params: { orderid: order.orderid } }" class="order-a"
                      title="View">View
                    </nuxt-link>
                  </span>
                </div>
...
export default {
  ...
  methods: {
    ...
    async getOrders(page = 1) {
      ...
      let response = await this.$axios.$get("orderdetail", {
        params: {
        ...
        this.modvalue = this.orders.length % 4;
      this.page_count = response.last_page;
    ...
    async search(orderpage = 1) {
      ...
      let response = await this.$axios.$post('searchorder/' + this.searchquery + '?' + '&page=' + orderpage)
      this.orders = response.data;
      this.page_count = response.last_page;
      this.total = response.total;
    },
