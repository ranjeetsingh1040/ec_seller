          ...
          <div class="columns mb-2 lg:mb-10">
            <div class="field column flex flex-col lg:flex-row">
              ...
              <div class="control w-full lg:w-3/4">
                <input class="input" :class="{ 'is-danger': errors.name }" type="text" v-model="viewstore.name"
                  autofocus="" id="name">
          ...
          <div class="columns mt-2 lg:mt-0 mb-2 lg:mb-10">
            <div class="field column  flex flex-col lg:flex-row">
              ...
              <div class="onoffswitch">
                <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" @change="onChangeEventHandler(viewstore.status)" id="myonoffswitch" checked
                  v-model="viewstore.status" :class="{ 'is-danger': errors.status }">
          ...
          <div class="columns mb-2 lg:mb-10">
            <div class="column">
              <div class="field flex flex-col lg:flex-row">
                ...
                <div class="control w-full lg:w-3/4">
                  <textarea class="form-control" rows="5" cols="" :class="{ 'is-danger': errors.description }"
                    v-model="viewstore.description">
            ...
            <div class="field column">
              ...
              <div class="drop display-inline align-center h-32" @dragover.prevent @drop="onDrop">
                ...
                <label v-if="!logoimage" class="btn display-inline">Add Store Logo
                  <input type="file" name="logoimage" v-on:change="onImageChange"
                    :class="{ 'is-danger': errors.storelogo }">
                </label>
                <div class="hidden display-inline align-center" v-else v-bind:class="{ 'image': true }">
                  <div class="upload-bg" v-bind:style="{ backgroundImage: 'url(' + logoimage + ')' }">
                    ...
                    <a href="javascript:void(0)" class="remove-btn float-right" @click="removeFile">
                      <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 47.971 47.971"
                        xml:space="preserve" class="w-2 h-2 fill-current m-1 ">
                        <g>
            ...
            <div class="field column">
              ...
              <div class="drop display-inline align-center h-32" @dragover.prevent @drop="onCoverDrop">
                ...
                <label v-if="!coverimage" class="btn display-inline">Add Cover Logo
                  <input type="file" name="coverimage" v-on:change="onCoverImageChange"
                    :class="{ 'is-danger': errors.storeimage }">
                </label>
                <div class="hidden display-inline align-center" v-else v-bind:class="{ 'image': true }">
                  <div class="upload-bg" v-bind:style="{ backgroundImage: 'url(' + coverimage + ')' }">
                    <!--  <img :src="coverimage" alt="" class="img w-32 h-32"  /> -->

                    <a href="javascript:void(0)" class="remove-btn float-right" @click="removeCoverFile">
                      <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 47.971 47.971"
                        xml:space="preserve" class="w-2 h-2 fill-current m-1 ">
                        <g>
...
export default {
  ...
  methods: {
    ...
    async store() {
      ...
      let responses = await this.$axios.$post('stores/editstores/' + this.storeId, formData, {
        headers: {
    ...
    async getStoredata() {
      ...
      this.viewstore.status = response.status
      this.logoimage = process.env.SITEURL + this.viewstore.storelogo;
      this.coverimage = process.env.SITEURL + this.viewstore.storeimage;
    },
    //preview start
    onDrop: function (e) {
      e.stopPropagation();
    ...
    removeFile() {
      if (typeof this.logoimage != 'undefined') {
        this.logoimage = '';
      }
      if (typeof this.viewstore.storelogo != 'undefined') {
        this.viewstore.storelogo = '';
      }
    },
    removeCoverFile() {
      if (typeof this.coverimage != 'undefined') {
        this.coverimage = '';
      }
      if (typeof this.viewstore.storeimage != 'undefined') {
        this.viewstore.storeimage = '';
      }
    },
    cancel() {
      this.$router.replace({
    ...
    onChangeEventHandler(id){
      if(id===true){
        this.viewstore.status = 1;
      }
      else if(id===false){
        this.viewstore.status = 0;
      }
      },
