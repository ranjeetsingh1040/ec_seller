  ...
  <div id="app">
    ...
    <div v-bind:class="[this.product_tab == 2 ? 'block' : 'hidden']">
      <form action="#" @submit.prevent="store" name="myForm" id="myForm" class="custom-form">
        ...
        <!-- <div v-if="this.success != null" class="w-1/2 my-4 bg-red-500 p-4 flex justify-between" id="success-alert">
        ...
        <div v-if="this.success != null" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative"
          role="alert">
          ...
          <div class="mb-5" v-for="(attributeproduct, i) in attributeproducts" :key="attributeproduct.id">
            <div class="w-full">
              <div class="flex">
                <div class="w-1/3 mb-5">
                  ...
                  <p class="font-bold">
                    {{ attributeproduct.attribute_label }}
                  </p>
                </div>
                ...
                <template v-if="attributeproduct.input_type == 'checkbox'">
                  <div class="w-2/3 mb-5">
                    <input type="checkbox" v-model="form.checkboxtype[i]" :value="attributeproduct.attribute_value"
                      ref="form.checkboxtype[i]" name="chkweight" :id="'chk_' + attributeproduct.attribute_value" />{{
           ...
           <div class="w-2/3 mb-5">
             <div class="field  columns">
                  ...
                  <div class="w-2/3 mb-5">
                    <div class="field column is-5">
                      <select class="select is-fullwidth" @change="changed" v-model="form.selecttype[i]">
                        ...
                        <option :value="attributeproduct.attribute_value" :key="attributeproduct.attribute_value">
                          {{ attributeproduct.attribute_value }}
                        </option>
                      </select>
                    </div>
                ...
                <div class="w-2/3 mb-5">
                  <p class="font-bold">
                    <input type="checkbox" v-model="form.variation[i]" ref="form.variation[i]" name="chkvariation"
                      :id="'chk_' + attributeproduct.id" />
                    Is Variation
                  </p>
                  <div class="field columns" v-bind:class="[
                    form.variation[i] == true ? 'block' : 'disp-none',
                  ]">
                    <div class="control">
                      <p>Price</p>
...
export default {
  ...
  methods: {
    async store() {
      for (let i = 0; i < this.attributeproducts.length; i++) {
        if (this.form.checkboxtype[i] == true) {
          this.productvariation.push({
            ...
            order: i,
            price: this.price,
            isvariation: this.form.variation[i],
            isprice: this.form.isprice[i],
            stock: this.form.stock[i],
          });
        }

        if (this.attributeproducts[i].input_type == "dropdown") {
          if (
            this.form.selecttype[i] == this.attributeproducts[i].attribute_value
          ) {
            this.productvariation.push({
              attribute_product_id: this.attributeproducts[i].id,
              attribute_value: this.attributeproducts[i].attribute_value,
        ...
        if (
          this.attributeproducts[i].input_type == "text" &&
      ...
      let response = await this.$axios
        .$post("attributes/attributeproductstore", this.productvariation)
        .then((response) => {
  ...
  created() {
    ...
    this.$nuxt.$on("productprice", (data) => {
